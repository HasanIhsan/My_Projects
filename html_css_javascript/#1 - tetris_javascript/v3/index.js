...
breaksound.setAttribute('src', './assets/breaksound.mp3');
    ...
    for(let i = 0; i < rotatedShape.length; i++) {
        for(let j = 0; j < rotatedShape[i].length; j++) {
            if(rotatedShape[i][j] !== 0) {
                ...
                if( row >= BOARD_HEIGHT || col < 0 || col >= BOARD_HEIGHT || (row >= 0 && board[row][col] !== 0 ) ) {
                    return false;
                }
            }
        }
